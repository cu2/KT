...
def suggest_film(request):
    if request.POST:
        ...
        if film_imdb_link.startswith('tt'):
            ...
            film_imdb_link = ''
        film_porthu_link = kt_utils.strip_whitespace(request.POST.get('film_porthu_link', ''))
        film_porthu_link = kt_utils.parse_porthu_link(film_porthu_link)
...
def films_with_missing_data(request):
    ...
    if miss_type == 'tortenet':
        ...
        films = models.Film.objects.filter(main_roles_confirmed=False, number_of_actors__gt=0).filter(genre_cache_is_animation=False, genre_cache_is_short=False, genre_cache_is_docu=False).order_by('-number_of_ratings')[offset_min:offset_max]
